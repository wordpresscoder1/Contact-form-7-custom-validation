<?php
/*
Plugin Name: Product Custome Table
Description: Product Custome Table
Plugin URI: 
Author URI: 
Author: Mahesh Patel
License: GPL
...
function custom_table_example_install()
{
    ...
    $table_name = $wpdb->prefix . 'product_custome'; // do not forget about tables prefix
    $table_name_cat = $wpdb->prefix . 'category_custome'; // do not forget about tables prefix
...
function custom_table_example_install_data()
{
    ...
    $table_name = $wpdb->prefix . 'product_custome'; // do not forget about tables prefix
	$table_name_cat = $wpdb->prefix . 'category_custome';
	...
	$wpdb->insert($table_name_cat, array(
        'name' => 'category1'
    ));
    $wpdb->insert($table_name_cat, array(
        'name' => 'category2'        
...
class Custom_Table_Category_List_Table extends WP_List_Table
{
...
*/
    ...
    function column_name($item)
    {
        ...
        $actions = array(
            'edit' => sprintf('<a href="?page=category_form&id=%s">%s</a>', $item['id'], __('Edit', 'custom_table_example')),
            'delete' => sprintf('<a href="?page=%s&action=delete&id=%s">%s</a>', $_REQUEST['page'], $item['id'], __('Delete', 'custom_table_example')),
        );
    ...
    {
        global $wpdb;
        $table_name = $wpdb->prefix . 'category_custome'; // do not forget about tables prefix
...
function custom_table_example_admin_menu()
{
	...
	add_submenu_page('persons','Categories','category', 'activate_plugins', 'category', 'custom_table_example_category_page_handler');
	add_submenu_page('persons', __('Add new', 'custom_table_example'), __('Add new', 'custom_table_example'), 'activate_plugins', 'category_form', 'custom_table_example_category_form_page_handler');
}
...
function custom_table_example_category_page_handler()
{
    global $wpdb;

    $table = new Custom_Table_Category_List_Table();
    $table->prepare_items();

    $message = '';
    if ('delete' === $table->current_action()) {
        $message = '<div class="updated below-h2" id="message"><p>' . sprintf(__('Category deleted: %d', 'custom_table_example'), count($_REQUEST['id'])) . '</p></div>';
    }
    ?>
<div class="wrap">
...
function custom_table_example_category_form_page_handler()
{
    global $wpdb;
    $table_name = $wpdb->prefix . 'category_custome'; // do not forget about tables prefix
    ...
    if (wp_verify_nonce($_REQUEST['nonce'], basename(__FILE__))) {
        ...
        // validate data, and if all ok save item to database
        // if id is zero insert otherwise update
        $item_valid = custom_table_category_validate_person($item);
        if ($item_valid === true) {
            if ($item['id'] == 0) {
                $result = $wpdb->insert($table_name, $item);
                $item['id'] = $wpdb->insert_id;
                if ($result) {
                    $message = __('Category was successfully saved', 'custom_table_example');
                } else {
                    $notice = __('There was an error while saving category', 'custom_table_example');
                }
            } else {
                $result = $wpdb->update($table_name, $item, array('id' => $item['id']));
                if ($result) {
                    $message = __('Category was successfully updated', 'custom_table_example');
                } else {
                    $notice = __('There was an error while updating category', 'custom_table_example');
                }
            }
        } else {
            // if $item_valid not true it contains error message(s)
        ...
        if (isset($_REQUEST['id'])) {
            ...
            if (!$item) {
                $item = $default;
                $notice = __('Category not found', 'custom_table_example');
            }
        }
    }

    // here we adding our custom meta box
    add_meta_box('category_form_meta_box', 'Category data', 'custom_table_example_category_form_meta_box_handler', 'category', 'normal', 'default');

    ?>
<div class="wrap">
    <div class="icon32 icon32-posts-post" id="icon-edit"><br></div>
    <h2><?php _e('Category', 'custom_table_example')?> <a class="add-new-h2"
                                href="<?php echo get_admin_url(get_current_blog_id(), 'admin.php?page=category');?>"><?php _e('back to list', 'custom_table_example')?></a>
    </h2>
    ...
    <form id="form" method="POST">
        ...
        <div class="metabox-holder" id="poststuff">
            <div id="post-body">
                <div id="post-body-content">
                    <?php /* And here we call our custom meta box */ ?>
                    <?php do_meta_boxes('category', 'normal', $item); ?>
                    <input type="submit" value="<?php _e('Save', 'custom_table_example')?>" id="submit" class="button-primary" name="submit">
                </div>
            </div>
        </div>
...
<table cellspacing="2" cellpadding="5" style="width: 100%;" class="form-table">
    ...
    <tr class="form-field">
        ...
        <td>
            <input id="name" name="name" type="text" style="width: 95%" value="<?php echo esc_attr($item['name'])?>"  size="50" class="code" placeholder="<?php _e('Your name', 'custom_table_example')?>" required>
        </td>
    </tr>
    <tr class="form-field">
        <th valign="top" scope="row">
...
function custom_table_example_category_form_meta_box_handler($item)
{
    ?>
...
function custom_table_category_validate_person($item)
{
    $messages = array();
